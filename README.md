...
- **手動設定**: `.env` ファイルを自分で作成し、`QIITA_TOKEN=あなたのトークン` と記述することでも設定可能です。
...
[analyze.py](analyze.py) を使用し、取得済みの JSON を集計・可視化できます。

```bash
# 例: search.py の出力JSONを分析
python analyze.py qiita_search_res_20260410_120000.json

# 出力先と上位タグ件数を指定
python analyze.py qiita_items_qiita_20260410_120000.json --out-dir analysis_output --top-tags 15
```
